...
(() => {
  ...
  function initHeroTvAscii() {
    ...
    // Track last canvas size — only resize when it actually changes
    let lastW = 0, lastH = 0;
    ...
    // Cache stable uniforms outside the render loop.
    // These only change when params is mutated (via setHeroTvAscii).
    // Set them once here, and again whenever setHeroTvAscii is called.
    function uploadStableUniforms() {
    ...
    // Patch setHeroTvAscii so stable uniforms re-upload on any change
    const _originalSet = window.setHeroTvAscii;
    window.setHeroTvAscii = function(newParams) {
      Object.assign(params, newParams);
      uploadStableUniforms();
    };
    ...
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    const cellUniform = params.cellSize * dpr;
    ...
    function render() {
      ...
      if (video.readyState >= 2) {
        ...
        // Only u_time changes every frame
