 ...
 * 2. Editorial 2-Column Grid Layout & Spacing
 * 3. Card 01 (DAY ONE® RUN) Sizing, Position & Typography
 * 4. Card 02 (AARDVARK) Sizing, Position & Typography
 * 5. Card 03 (RODEWALD) & Giant Watermark 25
 * 6. Contact Info Coordinates (Address, Phone, Email)
 * 7. THE BEST OF N/P & Portfolio Header
...
(function () {
  ...
  const DEFAULT_CONFIG = {
    ...
    // 2. Editorial 2-Column Grid Layout
    ...
    // 5. Card 03 (RODEWALD) & Watermark 25
    ...
    // 6. Contact Coordinates (Address, Phone, Email)
  ...
  class VisualDesigner {
    ...
    loadSavedState() {
      ...
      ['np_visual_designer_v1','np_visual_designer_v2','np_visual_designer_v3',
       ...
       'np_visual_designer_v10','np_visual_designer_v11','np_visual_designer_v12'].forEach(k => localStorage.removeItem(k));

      this.hasSavedState = false;
      try {
        const saved = localStorage.getItem('np_visual_designer_v13');
        if (saved) {
    ...
    saveState() {
      try {
        localStorage.setItem('np_visual_designer_v13', JSON.stringify(this.state));
      } catch (e) {}
    }
    ...
    applyAll() {
      // Only inject inline CSS vars if user has previously saved custom state.
      // On fresh load, let the CSS :root values in futurethree-scroll.css take
      // natural effect — avoids overriding carefully-calibrated layout values.
