...
(function () {
  ...
  const STORAGE_KEY = 'np_manifesto_work_margins_v17';

  const DESIGNER_CONFIG = {
    // ── TAB 0: HERO TEXT CONTROLS ─────────────────────────────────────────────
    '--hero-bottom-text-size':          { val: 48,  unit: 'px', min: 16,  max: 120, step: 1, label: 'Hero Text Font Size ("Nothing here/")', type: 'css' },
    '--hero-bottom-text-weight':        { val: 400, unit: '',   min: 100, max: 900, step: 100, label: 'Hero Text Boldness (Weight)',        type: 'css' },

    // ── TAB 1: MANIFESTO LOCKUP & STATEMENT ──────────────────────────────────
    '--manifesto-font-size':            { val: 43,  unit: 'px', min: 16,  max: 90,  step: 1, label: 'Statement Font Size',                 type: 'css' },
    '--manifesto-font-weight':          { val: 300, unit: '',   min: 100, max: 900, step: 100, label: 'Statement Boldness (Weight)',          type: 'css' },
    '--manifesto-letter-spacing':       { val: -1.8, unit: 'px', min: -5, max: 5,   step: 0.1, label: 'Statement Letter Spacing',          type: 'css' },
    '--manifesto-top-margin':           { val: 30,  unit: 'px', min: 0,   max: 250, step: 2, label: 'Section Top Padding',                 type: 'css' },
    '--manifesto-lockup-margin-top':    { val: 0,   unit: 'px', min: -50, max: 150, step: 1, label: 'Intro Lockup Margin Top',            type: 'css' },
    '--manifesto-gap':                  { val: 210, unit: 'px', min: 0,   max: 500, step: 5, label: 'Intro Lockup Bottom Margin',         type: 'css' },
    '--manifesto-coord-margin-top':     { val: 2,   unit: 'px', min: -40, max: 100, step: 1, label: 'Coordinates Top Offset',             type: 'css' },
    ...
    '--intro-divider-margin-bottom':    { val: 24,  unit: 'px', min: 0,   max: 100, step: 1, label: 'Separation Lines to Featured Gap',    type: 'css' },
    '--intro-divider-thickness':        { val: 1,   unit: 'px', min: 0.5, max: 8,   step: 0.5, label: 'Separation Lines Thickness',        type: 'css' },
    '--featured-tag-font-size':         { val: 25,  unit: 'px', min: 10,  max: 90,  step: 1, label: 'Featured works Font Size',              type: 'css' },
    '--featured-tag-font-weight':       { val: 400, unit: '',   min: 100, max: 900, step: 100, label: 'Featured works Boldness (Weight)',   type: 'css' },
    ...
    '--works-grid-margin-top':          { val: 20,  unit: 'px', min: -60, max: 150, step: 2, label: 'Works Grid Margin Top',                 type: 'css' },
    ...
    '--card-tags-margin-top':           { val: 8,   unit: 'px', min: -15, max: 35,  step: 1, label: 'Title to Tags Gap',                    type: 'css' },
    ...
    // ── TAB 5: NAVBAR PILL SIZE & POSITION ────────────────────────────────────
    '--nav-box-width':                  { val: 480, unit: 'px', min: 280, max: 900, step: 5, label: 'Navbar Width / Length',               type: 'css' },
    '--nav-box-height':                 { val: 56,  unit: 'px', min: 28,  max: 120, step: 1, label: 'Navbar Height (Pill Box Height)',       type: 'css' },
    '--nav-box-pady':                   { val: 10,  unit: 'px', min: 0,   max: 40,  step: 1, label: 'Navbar Vertical Padding (Internal)',     type: 'css' },
    '--nav-box-padx':                   { val: 17,  unit: 'px', min: 0,   max: 60,  step: 1, label: 'Navbar Side Margins (Padding X)',      type: 'css' },
    '--nav-box-gap':                    { val: 25,  unit: 'px', min: 0,   max: 80,  step: 1, label: 'Navbar Items Spacing Gap',             type: 'css' },
    '--nav-box-radius':                 { val: 4,   unit: 'px', min: 0,   max: 40,  step: 1, label: 'Navbar Corner Curvature (Radius)',     type: 'css' },
    '--nav-box-top':                    { val: 25,  unit: 'px', min: 0,   max: 100, step: 1, label: 'Navbar Distance from Top',             type: 'css' },
  };
  ...
  const CATEGORIES = [
    {
      ...
      title: 'Hero',
      keys: [
        '--hero-bottom-text-size',
        '--hero-bottom-text-weight'
      ]
    },
    {
      id: 'manifesto_tab',
    ...
    },
    {
      ...
      title: 'Featured',
      keys: [
        '--intro-divider-margin-bottom',
        '--intro-divider-thickness',
        '--featured-tag-font-size',
        '--featured-tag-font-weight',
  ...
  class VisualDesignerHUD {
    ...
    createUI() {
      ...
      header.innerHTML = `
        <div class="vd-title">
          <span>TYPOGRAPHY / WORK HUD</span>
          <span class="vd-drag-handle">⠿</span>
        </div>
        <div style="display:flex;align-items:center;gap:6px;">
          <button class="vd-jump-btn" id="vd-jump-hero" title="Jump to Hero" style="background:transparent;border:1px solid rgba(255,255,255,0.15);color:#00ff66;font-family:monospace;font-size:9px;padding:3px 6px;border-radius:3px;cursor:pointer;">↑ Hero</button>
      ...
      CATEGORIES.forEach((cat, idx) => {
        ...
        btn.addEventListener('click', () => {
          ...
          if (cat.id === 'hero_tab' || cat.id === 'navbar_tab') {
            window.scrollTo({ top: 0, behavior: 'smooth' });
          } else if (cat.id === 'manifesto_tab') {
            document.getElementById('f3-intro')?.scrollIntoView({ behavior: 'smooth' });
          } else if (cat.id === 'featured_grid_tab' || cat.id === 'cards_tab') {
        ...
        cat.keys.forEach(key => {
          ...
          ctrl.innerHTML = `
            <div class="vd-control-header">
              ...
              <span class="vd-control-value" data-key="${key}" id="${safeId}">${curVal}${unitStr}</span>
            </div>
          ...
          slider.addEventListener('input', e => {
            ...
            panel.querySelectorAll(`input[data-key="${key}"]`).forEach(inp => inp.value = n);
            panel.querySelectorAll(`.vd-control-value[data-key="${key}"]`).forEach(d => d.textContent = n + unitStr);
            this.applyVal(key, n);
          });
      ...
      header.querySelector('#vd-jump-hero')?.addEventListener('click', () => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });

      header.querySelector('#vd-jump-nav')?.addEventListener('click', () => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
      ...
      footer.querySelector('#vd-btn-reset').addEventListener('click', () => {
        ...
        panel.querySelectorAll('input[type="range"]').forEach(s => {
          ...
          const unitStr = DESIGNER_CONFIG[k].unit || '';
          panel.querySelectorAll(`.vd-control-value[data-key="${k}"]`).forEach(d => {
            d.textContent = DESIGNER_CONFIG[k].val + unitStr;
          });
        });
      });

      // Copy CSS Tokens
      const copyBtn = footer.querySelector('#vd-btn-copy');
      if (copyBtn) {
        copyBtn.addEventListener('click', async () => {
          ...
          // 3. Fallback to console + prompt if needed
          if (copied) {
            markCopied();
          } else {
            console.log('CSS Tokens:\n', css);
            prompt('Copy CSS Tokens below (Ctrl+C / Cmd+C):', css);
          }
        });
      }
